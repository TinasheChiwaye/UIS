...
{
    public partial class ImportExcelBackup : AdminBasePage
    {
        ...
        private void WriteToDatabase(DataTable dt, Guid newImportedId)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FuneralConnection"].ConnectionString))
            {
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(con))
                {
                    ...
                    bulkCopy.ColumnMappings.Add("PolicyNumber", "MemeberNumber");
        ...
        protected void btnConfirmAndSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                MembersBAL.MemberRowImportToMember(hdnMemberName.Value, Guid.Parse(hdnnewImportedId.Value));
